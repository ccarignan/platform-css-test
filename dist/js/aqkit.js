...
function transition(element, props, duration, timing) {
    ...
    return Promise$$1.all(toNodes(element).map(function (element) { return new Promise$$1(function (resolve, reject) {
            ...
            once(element, 'transitionend transitioncanceled', function (ref) {
                ...
                clearTimeout(timer);
                removeClass(element, 'aq-transition');
            ...
            addClass(element, 'aq-transition');
...
var Transition = {
    ...
    inProgress: function inProgress(element) {
        return hasClass(element, 'aq-transition');
    }

};

var animationPrefix = 'aq-animation-';
var clsCancelAnimation = 'aq-cancel-animation';
...
function componentAPI (aqUIkit) {

    var DATA = aqUIkit.data;

    aqUIkit.components = {};

    aqUIkit.component = function (id, options) {

        var name = camelize(id);

        if (isPlainObject(options)) {
            options.name = name;
            options = aqUIkit.extend(options);
        } else if (isUndefined(options)) {
            return aqUIkit.components[name];
        } else {
            options.options.name = name;
        }

        aqUIkit.components[name] = options;

        aqUIkit[name] = function (element, data) {
            var i = arguments.length, argsArray = Array(i);
            while ( i-- ) argsArray[i] = arguments[i];


            if (isPlainObject(element)) {
                return new aqUIkit.components[name]({data: element});
            }

            if (aqUIkit.components[name].options.functional) {
                return new aqUIkit.components[name]({data: [].concat( argsArray )});
            }
            ...
            function init(element) {
                ...
                var cmp = aqUIkit.getComponent(element, name);

                if (cmp) {
                    cmp.$reset(data);
                }

                return cmp || new aqUIkit.components[name]({el: element, data: data || {}});
            }

        };

        if (aqUIkit._initialized && !options.options.functional) {
            fastdom.read(function () { return aqUIkit[name](("[aq-" + id + "],[data-aq-" + id + "]")); });
        }

        return aqUIkit.components[name];
    };

    aqUIkit.getComponents = function (element) { return element && (element = isJQuery(element) ? element[0] : element) && element[DATA] || {}; };
    aqUIkit.getComponent = function (element, name) { return aqUIkit.getComponents(element)[name]; };

    aqUIkit.connect = function (node) {
        ...
        for (var i = 0; i < node.attributes.length; i++) {
            ...
            if (name && name in aqUIkit.components) {
                aqUIkit[name](node);
            }
    ...
    aqUIkit.disconnect = function (node) {
...
function getComponentName(attribute) {
    return startsWith(attribute, 'aq-') || startsWith(attribute, 'data-aq-')
        ? camelize(attribute.replace('data-aq-', '').replace('aq-', ''))
        : false;
}

function boot (aqUIkit) {

    var connect = aqUIkit.connect;
    var disconnect = aqUIkit.disconnect;
    ...
    function applyMutation(mutation) {
        ...
        update && aqUIkit.update(createEvent('update', true, false, {mutation: true}), target, true);
    ...
    function applyAttribute(ref) {
        ...
        var component = aqUIkit.getComponent(target, name);
    ...
    function apply(node, fn) {
        ...
        if (node.nodeType !== 1 || hasAttr(node, 'aq-no-boot')) {
            return;
        }
...
function globalAPI (aqUIkit) {

    var DATA = aqUIkit.data;

    aqUIkit.use = function (plugin) {
    ...
    aqUIkit.mixin = function (mixin, component) {
        component = (isString(component) ? aqUIkit.components[component] : component) || this;
    ...
    aqUIkit.extend = function (options) {

        options = options || {};

        var Super = this;
        var Sub = function aqUIkitComponent (options) {
            this._init(options);
        };
    ...
    aqUIkit.update = function (e, element, parents) {
    ...
    var container;
    Object.defineProperty(aqUIkit, 'container', {
...
function hooksAPI (aqUIkit) {

    aqUIkit.prototype._callHook = function (hook) {
        var this$1 = this;
    ...
    aqUIkit.prototype._callConnected = function () {
        var this$1 = this;
        ...
        if (!includes(aqUIkit.elements, this.$options.el)) {
            aqUIkit.elements.push(this.$options.el);
        }

        aqUIkit.instances[this._uid] = this;
    ...
    aqUIkit.prototype._callDisconnected = function () {
        ...
        var index = aqUIkit.elements.indexOf(this.$options.el);

        if (~index) {
            aqUIkit.elements.splice(index, 1);
        }

        delete aqUIkit.instances[this._uid];
    ...
    aqUIkit.prototype._callReady = function () {
    ...
    aqUIkit.prototype._callUpdate = function (e) {
        var this$1 = this;
...
function stateAPI (aqUIkit) {

    var uid = 0;

    aqUIkit.prototype.props = {};

    aqUIkit.prototype._init = function (options) {

        options = options || {};
        options = this.$options = mergeOptions(this.constructor.options, options, this);

        this.$el = null;
        this.$name = aqUIkit.prefix + hyphenate(this.$options.name);
        this.$props = {};
    ...
    aqUIkit.prototype._initData = function () {
        var this$1 = this;
    ...
    aqUIkit.prototype._initMethods = function () {
        var this$1 = this;
    ...
    aqUIkit.prototype._initComputeds = function () {
        var this$1 = this;
    ...
    aqUIkit.prototype._resetComputeds = function () {
        this._computeds = {};
    };

    aqUIkit.prototype._initProps = function (props) {
        var this$1 = this;
    ...
    aqUIkit.prototype._initEvents = function () {
        var this$1 = this;
    ...
    aqUIkit.prototype._unbindEvents = function () {
        this._events.forEach(function (unbind) { return unbind(); });
        this._events = [];
    };

    aqUIkit.prototype._initObserver = function () {
        var this$1 = this;
...
function instanceAPI (aqUIkit) {

    var DATA = aqUIkit.data;

    aqUIkit.prototype.$mount = function (el) {
    ...
    aqUIkit.prototype.$emit = function (e) {
        this._callUpdate(e);
    };

    aqUIkit.prototype.$update = function (e, parents) {
        aqUIkit.update(e, this.$options.el, parents);
    };

    aqUIkit.prototype.$reset = function (data) {
        this._callDisconnected();
        this._initProps(data);
        this._callConnected();
    };

    aqUIkit.prototype.$destroy = function (removeEl) {
...
var aqUIkit$3 = function (options) {
    this._init(options);
};

aqUIkit$3.util = util;
aqUIkit$3.data = '__uikit__';
aqUIkit$3.prefix = 'aq-';
aqUIkit$3.options = {};
aqUIkit$3.instances = {};
aqUIkit$3.elements = [];
...
hooksAPI(aqUIkit$3);
stateAPI(aqUIkit$3);
instanceAPI(aqUIkit$3);
componentAPI(aqUIkit$3);
...
var Container = {
    ...
    computed: {
        ...
        container: function container(ref) {
            ...
            return container === true && aqUIkit$3.container || container && $(container);
        }
            ...
            return new Promise$$1(function (resolve) {
                ...
                if (!this$1.queued || !isUndefined(animate) || !isUndefined(show) || !this$1.hasAnimation || targets.length < 2) {
                    ...
                    var body = doc.body,
                        ...
                        el = toggled[0],
                        inProgress = Animation.inProgress(el) && hasClass(el, 'aq-animation-leave')
        ...
        _toggleElement: function _toggleElement(el, show, animate) {
            ...
            show = isBoolean(show)
                ...
                : Animation.inProgress(el)
                    ? hasClass(el, 'aq-animation-leave')
...
function mixin (aqUIkit) {

    aqUIkit.mixin.class = Class;
    aqUIkit.mixin.container = Container;
    aqUIkit.mixin.modal = Modal;
    aqUIkit.mixin.position = Position;
    aqUIkit.mixin.togglable = Togglable;

}

function Accordion (aqUIkit) {

    aqUIkit.component('accordion', {
        ...
        defaults: {
            ...
            multiple: false,
            clsOpen: 'aq-open',
            toggle: '> .aq-accordion-title',
            content: '> .aq-accordion-content',
            transition: 'ease'
        },
...
function Alert (aqUIkit) {

    aqUIkit.component('alert', {
...
function Core (aqUIkit) {

    ready(function () {

        var scroll = 0, started = 0;

        on(win, 'load resize', aqUIkit.update);
        ...
        on(doc, 'animationstart', function (ref) {
            ...
            if ((css(target, 'animationName') || '').match(/^aq-.*(left|right)/)) {
                started++;
        ...
        Object.defineProperty(aqUIkit, 'hoverSelector', {
        ...
        aqUIkit.hoverSelector = '.aq-animation-toggle, .aq-transition-toggle, [aq-hover]';

    });

}

function Cover (aqUIkit) {

    aqUIkit.component('cover', {

        mixins: [Class, aqUIkit.components.video.options],
...
function Drop (aqUIkit) {

    var active;

    aqUIkit.component('drop', {
        ...
        computed: {
            ...
            clsDrop: function clsDrop(ref) {
                ...
                return clsDrop || ("aq-" + (this.$options.name));
            },
        ...
        connected: function connected() {
            ...
            this.toggle = toggle && aqUIkit.toggle(isString(toggle) ? query(toggle, this.$el) : this.$el.previousElementSibling, {target: this.$el, mode: this.mode});
    ...
    aqUIkit.drop.getActive = function () { return active; };
...
function Dropdown (aqUIkit) {

    aqUIkit.component('dropdown', aqUIkit.components.drop.extend({name: 'dropdown'}));

}

function FormCustom (aqUIkit) {

    aqUIkit.component('form-custom', {
...

            {
                ...
                handler: function handler(ref) {
                    ...
                    if (current === this.input) {
                        toggleClass(
                            this.state,
                            ("aq-" + (includes(type, 'focus') ? 'focus' : 'hover')),
...
function Gif (aqUIkit) {

    // Deprecated
    aqUIkit.component('gif', {
...
function Grid (aqUIkit) {

    aqUIkit.component('grid', aqUIkit.components.margin.extend({
        ...
        defaults: {
            margin: 'aq-grid-margin',
            clsStack: 'aq-grid-stack'
        },
...
function HeightMatch (aqUIkit) {

    aqUIkit.component('height-match', {
...
function HeightViewport (aqUIkit) {

    aqUIkit.component('height-viewport', {
...
function Icon (aqUIkit) {
    ...
    aqUIkit.component('icon', aqUIkit.components.svg.extend({
    ...
    [
        ...
        init: function init() {
            addClass(this.$el, 'aq-slidenav');

            if (hasClass(this.$el, 'aq-slidenav-large')) {
                this.icon += '-large';
            }
        }
    ...
    registerComponent('search-icon', {
        ...
        init: function init() {
            if (hasClass(this.$el, 'aq-search-icon') && parents(this.$el, '.aq-search-large').length) {
                this.icon = 'search-large';
            } else if (parents(this.$el, '.aq-search-navbar').length) {
    ...
    registerComponent('close', {
        ...
        init: function init() {
            this.icon = "close-" + (hasClass(this.$el, 'aq-close-large') ? 'large' : 'icon');
        }
    ...
    aqUIkit.icon.add = function (added) {
        Object.keys(added).forEach(function (name) {
            icons[name] = added[name];
            delete parsed[name];
        });

        if (aqUIkit._initialized) {
            each(aqUIkit.instances, function (component) {
    ...
    function registerComponent(name, mixin$$1) {
        ...
        aqUIkit.component(name, aqUIkit.components.icon.extend({
...
function Leader (aqUIkit) {

    aqUIkit.component('leader', {
        ...
        defaults: {
            fill: '',
            media: false,
            clsWrapper: 'aq-leader-fill',
            clsHide: 'aq-leader-hide',
            attrFill: 'data-fill'
        },
...
function Margin (aqUIkit) {

    aqUIkit.component('margin', {
        ...
        defaults: {
            margin: 'aq-margin-small-top',
            firstColumn: 'aq-first-column'
        },
...
function Modal$1 (aqUIkit) {

    aqUIkit.component('modal', {

        mixins: [Modal],

        defaults: {
            clsPage: 'aq-modal-page',
            clsPanel: 'aq-modal-dialog',
            selClose: '.aq-modal-close, .aq-modal-close-default, .aq-modal-close-outside, .aq-modal-close-full'
        },
        ...
        events: [

            {
                ...
                handler: function handler() {
                    ...
                    if (hasClass(this.panel, 'aq-margin-auto-vertical')) {
                        addClass(this.$el, 'aq-flex');
                    } else {
    ...
    aqUIkit.component('overflow-auto', {
        ...
        computed: {
            ...
            modal: function modal(_, $el) {
                return closest($el, '.aq-modal');
            },

            panel: function panel(_, $el) {
                return closest($el, '.aq-modal-dialog');
            }
    ...
    aqUIkit.modal.dialog = function (content, options) {

        var dialog = aqUIkit.modal((" <div class=\"aq-modal\"> <div class=\"aq-modal-dialog\">" + content + "</div> </div> "), options);
    ...
    aqUIkit.modal.alert = function (message, options) {

        options = assign({bgClose: false, escClose: false, labels: aqUIkit.modal.labels}, options);

        return new Promise$$1(
            function (resolve) { return on(aqUIkit.modal.dialog((" <div class=\"aq-modal-body\">" + (isString(message) ? message : html(message)) + "</div> <div class=\"aq-modal-footer aq-text-right\"> <button class=\"aq-button aq-button-primary aq-modal-close\" autofocus>" + (options.labels.ok) + "</button> </div> "), options).$el, 'hide', resolve); }
        );
    };

    aqUIkit.modal.confirm = function (message, options) {

        options = assign({bgClose: false, escClose: false, labels: aqUIkit.modal.labels}, options);

        return new Promise$$1(
            function (resolve, reject) { return on(aqUIkit.modal.dialog((" <div class=\"aq-modal-body\">" + (isString(message) ? message : html(message)) + "</div> <div class=\"aq-modal-footer aq-text-right\"> <button class=\"aq-button aq-button-default aq-modal-close\">" + (options.labels.cancel) + "</button> <button class=\"aq-button aq-button-primary aq-modal-close\" autofocus>" + (options.labels.ok) + "</button> </div> "), options).$el, 'click', '.aq-modal-footer button', function (ref) {
                var target = ref.target;
    ...
    aqUIkit.modal.prompt = function (message, value, options) {
        ...
        return new Promise$$1(function (resolve) {
            ...
            var resolved = false,
                prompt = aqUIkit.modal.dialog((" <form class=\"aq-form-stacked\"> <div class=\"aq-modal-body\"> <label>" + (isString(message) ? message : html(message)) + "</label> <input class=\"aq-input\" autofocus> </div> <div class=\"aq-modal-footer aq-text-right\"> <button class=\"aq-button aq-button-default aq-modal-close\" type=\"button\">" + (options.labels.cancel) + "</button> <button class=\"aq-button aq-button-primary\">" + (options.labels.ok) + "</button> </div> </form> "), options),
...
function Nav (aqUIkit) {

    aqUIkit.component('nav', aqUIkit.components.accordion.extend({
...
function Navbar (aqUIkit) {

    aqUIkit.component('navbar', {
        ...
        defaults: {
            dropdown: '.aq-navbar-nav > li',
            align: !isRtl ? 'left' : 'right',
            clsDrop: 'aq-navbar-dropdown',
            mode: undefined,
            offset: undefined,
            delayShow: undefined,
            delayHide: undefined,
        ...
        connected: function connected() {
            ...
            if (this.dropbar) {
                ...
                addClass(this.dropbar, 'aq-navbar-dropbar');

                if (this.dropbarMode === 'slide') {
                    addClass(this.dropbar, 'aq-navbar-dropbar-slide');
                }
            }
        ...
        update: function update() {
            ...
            aqUIkit.drop(
                $$(((this.dropdown) + " ." + (this.clsDrop)), this.$el).filter(function (el) { return !aqUIkit.getComponent(el, 'drop') && !aqUIkit.getComponent(el, 'dropdown'); }),
        ...
        methods: {
            ...
            getActive: function getActive() {
                var active = aqUIkit.drop.getActive();
...
function Offcanvas (aqUIkit) {

    aqUIkit.component('offcanvas', {
        ...
        defaults: {
            content: '.aq-offcanvas-content',
            mode: 'slide',
            flip: false,
            overlay: false,
            clsPage: 'aq-offcanvas-page',
            clsContainer: 'aq-offcanvas-container',
            clsPanel: 'aq-offcanvas-bar',
            clsFlip: 'aq-offcanvas-flip',
            clsContent: 'aq-offcanvas-content',
            clsContentAnimation: 'aq-offcanvas-content-animation',
            clsSidebarAnimation: 'aq-offcanvas-bar-animation',
            clsMode: 'aq-offcanvas',
            clsOverlay: 'aq-offcanvas-overlay',
            selClose: '.aq-offcanvas-close'
        },
...
function Responsive (aqUIkit) {

    aqUIkit.component('responsive', {

        props: ['width', 'height'],

        init: function init() {
            addClass(this.$el, 'aq-responsive-width');
        },
...
function Scroll (aqUIkit) {

    aqUIkit.component('scroll', {
...
function Scrollspy (aqUIkit) {

    aqUIkit.component('scrollspy', {
        ...
        defaults: {
            ...
            repeat: false,
            delay: 0,
            inViewClass: 'aq-scrollspy-inview'
        },
...

            {
                ...
                read: function read(els) {
                    ...
                    this.elements.forEach(function (el, i) {
                        ...
                        if (!elData) {
                            var cls = data(el, 'aq-scrollspy-class');
...
function ScrollspyNav (aqUIkit) {

    aqUIkit.component('scrollspy-nav', {
        ...
        update: [

            {
                ...
                read: function read() {
                    if (this.scroll) {
                        aqUIkit.scroll(this.links, {offset: this.offset || 0});
                    }
                }
...
function Sticky (aqUIkit) {

    aqUIkit.component('sticky', {
        ...
        defaults: {
            ...
            clsActive: 'aq-active',
            clsInactive: '',
            clsFixed: 'aq-sticky-fixed',
            clsBelow: 'aq-sticky-below',
            selTarget: '',
            widthElement: false,
            showOnUp: false,
            media: false,
        ...
        connected: function connected() {
            ...
            this.placeholder = $('<div class="aq-sticky-placeholder"></div>');
...
function Svg (aqUIkit) {

    aqUIkit.component('svg', {
...
function Switcher (aqUIkit) {

    aqUIkit.component('switcher', {
        ...
        defaults: {
            ...
            swiping: true,
            cls: 'aq-active',
            clsContainer: 'aq-switcher',
            attrItem: 'aq-switcher-item',
            queued: true
        },
        ...
        events: [

            {
                ...
                delegate: function delegate() {
                    return ((this.toggle) + ":not(.aq-disabled)");
                },
        ...
        methods: {
            ...
            show: function show(item) {
                ...
                for (var i = 0; i < length; i++, next = (next + dir + length) % length) {
                    if (!matches(this$1.toggles[next], '.aq-disabled, [disabled]')) {
...
function Tab (aqUIkit) {

    aqUIkit.component('tab', aqUIkit.components.switcher.extend({
        ...
        defaults: {
            media: 960,
            attrItem: 'aq-tab-item'
        },

        init: function init() {

            var cls = hasClass(this.$el, 'aq-tab-left')
                ? 'aq-tab-left'
                : hasClass(this.$el, 'aq-tab-right')
                    ? 'aq-tab-right'
                    : false;

            if (cls) {
                aqUIkit.toggle(this.$el, {cls: cls, mode: 'media', media: this.media});
            }
        }
...
function Toggle (aqUIkit) {

    aqUIkit.component('toggle', {

        mixins: [aqUIkit.mixin.togglable],
...
function Video (aqUIkit) {

    aqUIkit.component('video', {
...
function core (aqUIkit) {
    ...
    // core components
    aqUIkit.use(Toggle);
    aqUIkit.use(Accordion);
    aqUIkit.use(Alert);
    aqUIkit.use(Video);
    aqUIkit.use(Cover);
    aqUIkit.use(Drop);
    aqUIkit.use(Dropdown);
    aqUIkit.use(FormCustom);
    aqUIkit.use(HeightMatch);
    aqUIkit.use(HeightViewport);
    aqUIkit.use(Margin);
    aqUIkit.use(Gif);
    aqUIkit.use(Grid);
    aqUIkit.use(Leader);
    ...
    aqUIkit.use(Nav);
    aqUIkit.use(Navbar);
    aqUIkit.use(Offcanvas);
    aqUIkit.use(Responsive);
    aqUIkit.use(Scroll);
    aqUIkit.use(Scrollspy);
    aqUIkit.use(ScrollspyNav);
    aqUIkit.use(Sticky);
    aqUIkit.use(Svg);
    aqUIkit.use(Icon);
    aqUIkit.use(Switcher);
    aqUIkit.use(Tab);
...
function plugin(aqUIkit) {
    ...
    aqUIkit.component('countdown', {

        mixins: [aqUIkit.mixin.class],
...
function plugin$1(aqUIkit) {
    ...
    aqUIkit.component('grid-parallax', aqUIkit.components.grid.extend({
    ...
    aqUIkit.components.gridParallax.options.update.unshift({
...
function AnimationsPlugin (aqUIkit) {
...
function TransitionerPlugin (aqUIkit) {
...
function AutoplayMixin (aqUIkit) {
        ...
        update: [

            {
                ...
                write: function write() {
                    ...
                    toggleClass($$(this.navItemSelector, this.$el).concat(this.nav), 'aq-hidden', !this.maxIndex);
...
function plugin$5(aqUIkit) {
    ...
    aqUIkit.mixin.slider = {

        attrs: true,

        mixins: [AutoplayMixin(aqUIkit), DragMixin(aqUIkit), NavMixin(aqUIkit)],
        ...
        defaults: {
            ...
            percent: 0,
            clsActive: 'aq-active',
            clsActivated: 'aq-transition-active',
...
function plugin$4(aqUIkit) {

    if (plugin$4.installed) {
        return;
    }

    aqUIkit.use(plugin$5);

    var mixin = aqUIkit.mixin;
    var ref = aqUIkit.util;
    var assign = ref.assign;
    var fastdom = ref.fastdom;
    var isNumber = ref.isNumber;

    var Animations = AnimationsPlugin(aqUIkit),
        Transitioner = TransitionerPlugin(aqUIkit);

    aqUIkit.mixin.slideshow = {
...
function AnimationsPlugin$1 (aqUIkit) {
...
function plugin$3(aqUIkit) {

    if (plugin$3.installed) {
        return;
    }

    aqUIkit.use(plugin$4);

    var mixin = aqUIkit.mixin;
    var util = aqUIkit.util;
    var $ = util.$;
    ...
    var Animations = AnimationsPlugin$1(aqUIkit);

    aqUIkit.component('lightbox-panel', {
        ...
        defaults: {
            ...
            items: [],
            cls: 'aq-open',
            clsPage: 'aq-lightbox-page',
            selList: '.aq-lightbox-items',
            attrItem: 'aq-lightbox-item',
            pauseOnHover: false,
            velocity: 2,
            Animations: Animations,
            template: "<div class=\"aq-lightbox aq-overflow-hidden\"> <ul class=\"aq-lightbox-items\"></ul> <div class=\"aq-lightbox-toolbar aq-position-top aq-text-right aq-transition-slide-top aq-transition-opaque\"> <button class=\"aq-lightbox-toolbar-icon aq-close-large\" type=\"button\" aq-close aq-toggle=\"!.aq-lightbox\"></button> </div> <a class=\"aq-lightbox-button aq-position-center-left aq-position-medium aq-transition-fade\" href=\"#\" aq-slidenav-previous aq-lightbox-item=\"previous\"></a> <a class=\"aq-lightbox-button aq-position-center-right aq-position-medium aq-transition-fade\" href=\"#\" aq-slidenav-next aq-lightbox-item=\"next\"></a> <div class=\"aq-lightbox-toolbar aq-lightbox-caption aq-position-bottom aq-text-center aq-transition-slide-bottom aq-transition-opaque\"></div> </div>"
        },
        ...
        created: function created() {
            ...
            this.caption = $('.aq-lightbox-caption', this.$el);
        ...
        events: [
...

            {
                ...
                handler: function handler(_, item) {
                    ...
                    this.setItem(item, '<span aq-spinner></span>');
                    ...
                    // Video
                        ...
                        var video = $(("<video controls playsinline" + (item.poster ? (" poster=\"" + (item.poster) + "\"") : '') + " aq-video=\"autoplay: " + (this.videoAutoplay) + "\"></video>"));
                    ...
                    // Iframe
                        ...
                        this.setItem(item, ("<iframe class=\"aq-lightbox-iframe\" src=\"" + source + "\" frameborder=\"0\" allowfullscreen></iframe>"));
        ...
        methods: {
            ...
            setItem: function setItem(item, content) {
                ...
                aqUIkit.update(null, el);
            },

            setError: function setError(item) {
                this.setItem(item, '<span aq-icon="icon: bolt; ratio: 2"></span>');
            },
            ...
            showControls: function showControls() {
                ...
                addClass(this.$el, 'aq-active', 'aq-transition-active');

            },

            hideControls: function hideControls() {
                removeClass(this.$el, 'aq-active', 'aq-transition-active');
            }
    ...
    function getIframe(src, width, height, autoplay) {
        return ("<iframe src=\"" + src + "\" width=\"" + width + "\" height=\"" + height + "\" style=\"max-width: 100%; box-sizing: border-box;\" frameborder=\"0\" allowfullscreen aq-video=\"autoplay: " + autoplay + "\" aq-responsive></iframe>");
    }

}

function plugin$2(aqUIkit) {

    if (plugin$2.installed) {
        return;
    }

    aqUIkit.use(plugin$3);
    ...
    aqUIkit.component('lightbox', {
        ...
        methods: {
            ...
            _init: function _init() {
                return this.panel = this.panel || aqUIkit.lightboxPanel(assign({}, this.$props, {
...
function plugin$6(aqUIkit) {
    ...
    aqUIkit.component('notification', {
        ...
        defaults: {
            ...
            group: null,
            pos: 'top-center',
            clsClose: 'aq-notification-close',
            clsMsg: 'aq-notification-message'
        },

        created: function created() {

            if (!containers[this.pos]) {
                containers[this.pos] = append(aqUIkit.container, ("<div class=\"aq-notification aq-notification-" + (this.pos) + "\"></div>"));
            }

            var container = css(containers[this.pos], 'display', 'block');

            this.$mount(append(container,
                ("<div class=\"" + (this.clsMsg) + (this.status ? (" " + (this.clsMsg) + "-" + (this.status)) : '') + "\"> <a href=\"#\" class=\"" + (this.clsClose) + "\" data-aq-close></a> <div>" + (this.message) + "</div> </div>")
            ));
    ...
    aqUIkit.notification.closeAll = function (group, immediate) {
        each(aqUIkit.instances, function (component) {
...
function plugin$7(aqUIkit) {

    if (plugin$7.installed) {
        return;
    }

    aqUIkit.use(plugin$8);
...
function SliderReactive (aqUIkit) {
...
function TransitionerPlugin$1 (aqUIkit) {
...
function plugin$9(aqUIkit) {
    ...
    var toggleClass = ref.toggleClass;
    var toFloat = ref.toFloat;
    var Transitioner = TransitionerPlugin$1(aqUIkit);

    aqUIkit.component('slider', {

        mixins: [mixin.class, mixin.slider, SliderReactive(aqUIkit)],
        ...
        defaults: {
            center: false,
            sets: false,
            attrItem: 'aq-slider-item',
            selList: '.aq-slider-items',
            selNav: '.aq-slider-nav',
            Transitioner: Transitioner
        },
        ...
        update: {
            ...
            write: function write() {
                ...
                $$(("[" + (this.attrItem) + "],[data-" + (this.attrItem) + "]"), this.$el).forEach(function (el) {
                    ...
                    this$1.maxIndex && toggleClass(el, 'aq-hidden', isNumeric(index) && (this$1.sets && !includes(this$1.sets, toFloat(index)) || index > this$1.maxIndex));
                });
...
function AnimationsPlugin$2 (aqUIkit) {
...
function plugin$10(aqUIkit) {
    ...
    var Animations = AnimationsPlugin$2(aqUIkit);

    aqUIkit.component('slideshow', {

        mixins: [mixin.class, mixin.slideshow, SliderReactive(aqUIkit)],
        ...
        defaults: {
            ...
            minHeight: false,
            maxHeight: false,
            selList: '.aq-slideshow-items',
            attrItem: 'aq-slideshow-item',
            selNav: '.aq-slideshow-nav',
            Animations: Animations
        },
    ...
    aqUIkit.component('slideshow-parallax', {
        ...
        computed: {
            ...
            item: function item() {
                var slideshow = aqUIkit.getComponent(closest(this.$el, '.aq-slideshow'), 'slideshow');
...
function plugin$11(aqUIkit) {
    ...
    aqUIkit.component('sortable', {
        ...
        defaults: {
            ...
            animation: 150,
            threshold: 5,
            clsItem: 'aq-sortable-item',
            clsPlaceholder: 'aq-sortable-placeholder',
            clsDrag: 'aq-sortable-drag',
            clsDragState: 'aq-drag',
            clsBase: 'aq-sortable',
            clsNoDrag: 'aq-sortable-nodrag',
            clsEmpty: 'aq-sortable-empty',
            clsCustom: '',
            handle: false
        },
        ...
        methods: {
            ...
            start: function start(e) {
                ...
                this.drag = append(aqUIkit.container, this.placeholder.outerHTML.replace(/^<li/i, '<div').replace(/li>$/i, 'div>'));
    ...
    function getSortable(element) {
        return element && (aqUIkit.getComponent(element, 'sortable') || getSortable(element.parentNode));
    }
...
function plugin$12(aqUIkit) {
    ...
    aqUIkit.component('tooltip', {
        ...
        defaults: {
            ...
            title: '',
            delay: 0,
            animation: ['aq-animation-scale-up'],
            duration: 100,
            cls: 'aq-active',
            clsPos: 'aq-tooltip'
        },
        ...
        methods: {
            ...
            show: function show() {
                ...
                this.tooltip = append(aqUIkit.container, ("<div class=\"" + (this.clsPos) + "\" aria-hidden><div class=\"" + (this.clsPos) + "-inner\">" + (this.title) + "</div></div>"));
...
aqUIkit$3.use(plugin);
aqUIkit$3.use(plugin$1);
aqUIkit$3.use(plugin$2);
aqUIkit$3.use(plugin$6);
aqUIkit$3.use(plugin$7);
aqUIkit$3.use(plugin$9);
aqUIkit$3.use(plugin$10);
aqUIkit$3.use(plugin$11);
aqUIkit$3.use(plugin$12);
aqUIkit$3.use(plugin$13);

{
    boot(aqUIkit$3);
}

function plugin$14(aqUIkit) {
  var obj;
